...
Optional **hCaptcha**: set `NEXT_PUBLIC_HCAPTCHA_SITE_KEY` and enable **Authentication → Bot and abuse protection** in the Supabase dashboard (hCaptcha secret + same provider). Sign-in and sign-up send `captchaToken` only when this key is set.

For **hosted** Supabase: set **Authentication → URL configuration → Site URL** to your real production origin (e.g. `https://www.useboopy.com` if that is what users type). Under **Redirect URLs**, add **every** variant you use, e.g. **`https://www.useboopy.com/login`** and **`https://useboopy.com/login`** (plus previews). The sign-up flow sets `emailRedirectTo` to **the current browser origin** + `/login`, so a missing **www** (or extra **www**) in the allow list will break confirmation. Set **`NEXT_PUBLIC_APP_URL`** on Vercel for features that need a single canonical origin.

If sign-up shows **error sending confirmation email**: Supabase failed to send (or rejected the request before send). **URL:** add **both** `https://useboopy.com/login` **and** `https://www.useboopy.com/login` if you use either host; set **Site URL** to the primary one. **Mail:** open **Dashboard → Logs** (filter Auth/API), then **Authentication → Emails** — custom SMTP mistakes cause this often; confirm you are not over [Auth rate limits](https://supabase.com/docs/guides/auth/rate-limits). **Local** `supabase start` uses **Inbucket** and often disables confirmations in `config.toml`; **hosted** projects need a working mail path. See [Supabase CAPTCHA / auth email](https://supabase.com/docs/guides/auth/auth-captcha).
